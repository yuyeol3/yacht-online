...
Redis는 API EC2 내부에서 실행되며, Game 서버 상태 확인, 방 목록 관리, 서버 간 상태 공유를 위한 임시 데이터 저장소로 사용했습니다. 여러 Game 서버의 상태를 공유한 후에는 API 서버가 Redis를 읽어 방 개수 기준으로 부하가 적은 Game 서버를 선택하되, TTL을 통해 heartbeat과 유사한 방식을 heartbeat가 끊긴 Game 서버는 선택하지 않도록 하였습니다.
