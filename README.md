...
Redis는 API EC2 내부에서 실행되며, Game 서버 상태 확인, 방 목록 관리, 서버 간 상태 공유를 위한 임시 데이터 저장소로 사용했습니다. 여러 Game 서버의 상태를 공유한 후에는 API 서버가 Redis를 읽어 부하가 적은 Game 서버를 선택하도록 구현하였습니다.
