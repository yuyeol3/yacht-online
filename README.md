...
1. **클라우드 인프라 기반 서비스 배포 경험**
	- 누구나 인터넷을 통해 24시간 접속 가능한 서비스 환경을 제공하기 위해 AWS 클라우드 인프라를 활용하였습니다.
	- 기존 로컬 개발 환경을 단순히 이전하는 것이 아닌, AWS의 관리형 서비스(S3, RDS, ALB, ACM)를 적극 활용하여 실제 서비스와 유사한 클라우드 배포 구조를 구성하였습니다.
	- 서버 역할 분리, Redis 기반 분산 상태 관리, Docker 컨테이너 기반 배포 구조를 적용하여 클라우드 환경에 적합한 아키텍처를 설계하였습니다.
	- 이를 통해 Cloud Native 아키텍처의 핵심 요소인 확장성과 고가용성을 고려한 서비스 구조를 일부 구축하였습니다.
2. **서버 역할 분리 경험**
...
ALB는 외부에서 들어오는 API 요청과 WebSocket 요청을 EC2 인스턴스의 백엔드 서버로 전달합니다. REST API 요청은 API 서버로, 실시간 게임 통신 요청은 Game 서버로 전달되도록 구성했습니다. 백엔드 EC2 인스턴스의 공인 IP를 외부에 직접 노출하지 않아 보안성을 향상시키며, AWS ACM과 연동하여 HTTPS 인증서를 중앙에서 관리할 수 있습니다. 향후 API 서버 또는 게임 서버 인스턴스를 추가할 경우 Target Group에 서버를 등록하는 것만으로 확장이 가능합니다.
...
Game 서버는 Spring Boot WebSocket 기반으로 구현되었으며, 실시간 게임 진행을 담당합니다. 사용자의 주사위 굴리기, 주사위 고정 및 해제, 점수 카테고리 선택, 턴 전환 등의 이벤트를 WebSocket STOMP 메시지로 처리합니다. 서버는 게임 룸 단위로 상태를 갱신하고, 같은 게임 룸의 모든 사용자에게 변경된 게임 상태를 브로드캐스트합니다. Game 서버는 2대를 두어 동시 게임 처리량 증가와 고가용성을 고려하였습니다. 
...
Redis는 API EC2 내부에서 실행되며, Game 서버 상태 확인, 방 목록 관리, 서버 간 상태 공유를 위한 임시 데이터 저장소로 사용했습니다. 여러 Game 서버의 상태를 공유한 후에는 API 서버가 Redis를 읽어 방 개수 기준으로 부하가 적은 Game 서버를 선택하되, TTL을 통해 heartbeat과 유사한 방식을 구현하여heartbeat가 끊긴 Game 서버는 선택하지 않도록 하였습니다.
...
https://yacht-online.yuyeol3.uk/ 로 접속하여 사용할 수 있습니다.
...
방장이 게임 시작 버튼을 누르면 게임이 시작됩니다. 왼쪽에는 누구의 턴인지 표시되며 남은 시간과 남은 주사위 굴림 횟수가 표시됩니다. Roll 버튼을 클릭하면 주사위를 굴릴 수 있습니다. 화면 오른쪽에는 점수판이 표시되며 현재 주사위 상태에 따라 얻을 수 있는 점수가 표시됩니다.
